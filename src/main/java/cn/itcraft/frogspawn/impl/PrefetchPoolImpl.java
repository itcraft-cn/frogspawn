...
public class PrefetchPoolImpl<T extends Resettable> extends AbstractPrefetchPoolImpl<T> {
     ...
     *
     * @return 获取到的泛型对象实例
     *         Retrieved generic object instance
     */
    @Override
    protected T fetchData() {
        return FetchHelper.fetchDataOrFailover(
            array,       // 对象存储数组 | Object storage array
            indexMask,   // 索引掩码用于快速取模 | Index mask for fast modulo operation
            walker,      // 数组遍历辅助工具 | Array traversal helper
            fetchFailStrategy,  // 失败处理策略 | Failure handling strategy
            creator      // 对象创建器 | Object creator
        );
    }
}
